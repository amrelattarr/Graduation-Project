...
export class VolunteerPickupTasks implements OnInit {
  ...
  loadOffers(showLoading = true): void {
    ...
    this.volunteerService.getPickupOffers().subscribe({
      next: (res: any) => {
        console.log('Raw offers response:', res);
  
        const data = this.extractList<VolunteerPickupOffer>(res);
  
        this.offers = data.map((offer: any) => ({
          ...offer,
          offeredAt: this.addHours(offer.offeredAt, 3),
          slaDueAt: this.addHours(offer.slaDueAt, 3)
        }));
  ...
  loadHistory(showLoading = true): void {
    ...
    this.volunteerService.getPickupHistory().subscribe({
      next: (res: any) => {
        ...
        const data = this.extractList<VolunteerPickupHistory>(res);
