...
export class CreateDonation {
  ...
  initForm(): void {
    this.donationForm = this.fb.group({
      ...
      address: ['', Validators.required], // new address field
  ...
  submit(): void {
    ...
    const latitude = Number(formValue.latitude);
    const longitude = Number(formValue.longitude);
    if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
      this.isSubmitting = false;
      this.errorMessage = 'Invalid location coordinates';
      return;
    }
    ...
    if (!Number.isFinite(charityId) || charityId <= 0) {
      this.isSubmitting = false;
      this.errorMessage = 'Invalid charity';
      return;
    }

    const address = (formValue.address ?? '').toString().trim();
    const description = (formValue.description ?? '').toString().trim();
    const notes = (formValue.notes ?? '').toString().trim();
    ...
    // helps correlate frontend request with backend logs
    console.log('CREATE DONATION payload (query params):', body);

    this.donor.createDonation(charityId, body, this.selectedImage).subscribe({
      next: () => {
        this.isSubmitting = false;
