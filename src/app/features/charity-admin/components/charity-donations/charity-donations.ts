...
export class CharityDonations {
  ...
  ngOnInit(): void {
    ...
    this.notificationService.realtimeUpdates$
      ...
      .subscribe(() => this.getPendingDonations());
  }
  ...
  getPendingDonations(): void {
    ...
    this.charityService.getPendingDonations().subscribe({
      next: (res) => {
        console.log('Pending donations loaded:', res.data);
  
        this.donations = (res.data ?? []).map(donation => ({
          ...donation,
          createdAt: this.addHours(donation.createdAt, 3).toISOString(),
          expirationTime: this.addHours(donation.expirationTime, 3).toISOString()
        }));
